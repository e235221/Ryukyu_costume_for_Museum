...
AR画面右上で「なし」「海」「石畳」を選ぶ。海は `background_beach.jpg`、石畳は `background_ishidatami.jpg` を使用する。`background.js` が人物を切り抜き、背景写真と合成し、その上に衣装を描画する。

MediaPipe Selfie Segmentation 0.1.1675465747の軽量なlandscapeモデルを必要時だけCDNから読み込み、推論を最大10fpsに制限する。カメラ映像は端末内で処理する。近距離向けの方式なので、遠距離や髪・指の輪郭では切り抜きが不安定になる場合がある。「なし」およびAR終了で背景推論を停止する。背景の初回使用にはインターネット接続が必要。

背景の選択と衣装の選択は独立している。現在の顔ハメは1人用。複数人対応には顔認識・人物ごとの衣装管理の改修が必要。

背景処理の模擬検証は `node files/check-background.cjs`。2026-09-19時点で切替・停止の検証はPASS。実カメラでの合成結果はブラウザ操作が拡張機能UIに阻まれたため未確認。

技術選定理由: 既存のMindAR顔追跡を維持しながら、軽量な人物マスクとCanvas 2Dで背景を合成できるため。背景の比率を維持し、画面を埋めるよう中央でトリミングする。
